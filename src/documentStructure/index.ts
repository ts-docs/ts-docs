 ...
 * like to use [React Static](https://github.com/react-static/react-static) instead of [eta](https://github.com/eta-dev/eta), then write
 ...
 * ### index.eta
 * 
 * `index.eta` contains the structure of EVERY file generated by the generator.
 * 
 * ### components folder
 * 
 * The generator pre-compiles all the components in the `components` folder and uses them. It does it like this
 * because we believe that there should be almost no logic inside your templates. Each template should
 * be short and easily read, without much javascript. The rendered HTML from the component is then passed to
 * other components or the `index.eta` file directly. For example, the generation process for a single class is this:
 * 
 ...
 * 
 * ```hbs
 * <% for (const property of it.properties) { %>
 *      <div><%= property%></div>
 * <% } %>
 * ```
 ...
 * ```
 * 
 * The first parameter is the Eta object, the second is an object with all the components.
 * 
 * ### assets folder
 * 
...
export interface DocumentStructure {
    path: string,
    components: Record<Components, BoundTemplateFunction>,
    index: BoundTemplateFunction,
    config: ExtendedEtaConfig
}

export type BoundTemplateFunction = (args: Record<string, unknown>) => string;

export interface ExtendedEtaConfig extends EtaConfig {
    hasChangelog: boolean
}

function createBoundFn(func: TemplateFunction, config: EtaConfig) {
    return (data: Record<string, unknown>) => func(data, config); 
}


export function setupDocumentStructure(structName: string, staticOptions: Record<string, unknown>) : DocumentStructure {
    ...
    for (const partialFile of fs.readdirSync(partials).filter(f => f.endsWith(".eta"))) {
        const fileContents = fs.readFileSync(path.join(partials, partialFile), "utf-8");
        Eta.templates.define(partialFile.slice(0, -4), Eta.compile(fileContents));
    }
    ...
    const etaOptions = Eta.getConfig({...staticOptions as EtaConfig, rmWhitespace: true});
    const components: Record<string, BoundTemplateFunction> = {};
    for (const componentFile of fs.readdirSync(componentsPath).filter(f => f.endsWith(".hbs"))) {
        components[componentFile.slice(0, -4)] = createBoundFn(Eta.compile(fs.readFileSync(path.join(componentsPath, componentFile), "utf8")), etaOptions);
    }
    ...
    if (fs.existsSync(helpers)) require(helpers)(Eta, components);
    return {
        path: pathToStructure,
        components,
        index: createBoundFn(Eta.compile(fs.readFileSync(index, "utf8")), etaOptions),
        config: etaOptions as ExtendedEtaConfig
    };
}
